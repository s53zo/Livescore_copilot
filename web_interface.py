...
def get_scores():
    try:
        ...
        for station in stations:
            ...
            # Get band breakdown
            band_breakdown = reporter.get_band_breakdown_with_rate(station_id, callsign_val, contest)
            
            # Get total QSO rate
            total_rate = reporter.get_total_qso_rate(station_id, callsign_val, contest)
            ...
            scores.append({
                ...
                'rate': total_rate,
                'bands': {
                    band: reporter.format_band_data(data) 
                    for band, data in band_breakdown.items()
                },
                'highlight': callsign_val == callsign,
                'totalQsos': qsos,
                'totalMults': mults
            })
        ...
        logger.error("Exception in get_scores:")
        logger.error(traceback.format_exc())
